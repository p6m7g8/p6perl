...
sub debug_dumper { print STDERR "DEBUG: $_[0] - " . Data::Dumper::Dumper $_[1] if $Debug  }
...
sub lwp {
    ...
    local $ENV{PERL_LWP_SSL_VERIFY_HOSTNAME} = $args{ssl_verify} || 1;
    ...
    debug_dumper("response", $response);
    error($@) if $@;
